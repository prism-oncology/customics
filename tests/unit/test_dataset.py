...
class TestMultiOmicsDataset:
    def test_len(self, mdata, clinical_df, sample_ids):
        ds = MultiOmicsDataset(mdata, sample_ids, clinical_df["label"])
        assert len(ds) == len(sample_ids)

    def test_getitem_shapes(self, mdata, clinical_df, sample_ids):
        ds = MultiOmicsDataset(mdata, sample_ids, clinical_df["label"])
    ...
    def test_getitem_dtype(self, mdata, clinical_df, sample_ids):
        ds = MultiOmicsDataset(mdata, sample_ids, clinical_df["label"])
        omics_tensors, _, _, _ = ds[0]
        for t in omics_tensors:
            assert t.dtype == torch.float32

    def test_no_label(self, mdata, clinical_df, sample_ids):
        ds = MultiOmicsDataset(mdata, sample_ids, None)
        _, lbl, _, _ = ds[0]
        assert lbl == 0

    def test_get_samples(self, mdata, clinical_df, sample_ids):
        ds = MultiOmicsDataset(mdata, sample_ids, clinical_df["label"])
