...
from customics import get_common_samples, get_sub_omics_df, prepare_input


class TestGetSubOmicsDf:
    def test_subsets_all_modalities(self, mdata, sample_ids):
        keep = sample_ids[:5]
        sub = get_sub_omics_df(mdata, keep)
    ...
    def test_carries_obs_and_uns(self, mdata, sample_ids):
        ...
        sub = get_sub_omics_df(mdata, sample_ids[:5])
    ...
    def test_ignores_unknown_samples(self, mdata, sample_ids):
        sub = get_sub_omics_df(mdata, [*sample_ids[:3], "NOT_A_SAMPLE"])
